		...
		public override void OnInspectorGUI()
		{
			...
			if (setupOpen)
			{
				...
				EditorGUILayout.BeginHorizontal();
					disabledGUIOnPlay();
					t.Steps=EditorGUILayout.IntField(t.Steps);
					if (t.Steps<4)
						t.Steps=4;
					DrakkarEditor.OverlappingLabel("Steps",Color.green);
					GUI.contentColor=Color.yellow;
					t.StepTime=(DrakkarTrail.STEPTIME)EditorGUILayout.EnumPopup(t.StepTime);
					GUI.contentColor=Color.white;
				EditorGUILayout.EndHorizontal();
				EditorGUILayout.BeginHorizontal();
					t.Bounds=EditorGUILayout.FloatField(t.Bounds);
					DrakkarEditor.OverlappingLabel("Bounds size",Color.green);
					t.Length=EditorGUILayout.FloatField(t.Length);
					DrakkarEditor.OverlappingLabel("Width",Color.green);
				EditorGUILayout.EndHorizontal();
			...
			if (eventsOpen)
			{
				...
				DrakkarEditor.ArrayGUI(so,"OnBegin","On Begin",.1f,false,true);
				GUI.backgroundColor=Color.green;
				DrakkarEditor.ArrayGUI(so,"OnEnd","On End",.1f,false,true);
				GUI.backgroundColor=Color.green;
				DrakkarEditor.ArrayGUI(so,"OnClear","On Clear",.1f,false,true);
			#else
