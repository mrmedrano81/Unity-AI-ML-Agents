...
namespace Drakkar
{
	...
	public class StaticList<T> where T : IStaticLister
	{
		...
		protected SimpleStaticList<int> removableItems;
		...
		public void BookRemoveSafe(IStaticLister ls)
		{
			if (removableHash.Add(ls))
				removableItems.Add(ls.StaticIndex);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Il2CppSetOption(Option.NullChecks,false)]
		public void BookRemove(int pos) => removableItems.Add(pos);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Il2CppSetOption(Option.NullChecks,false)]
		public void BookRemove(IStaticLister ls) => removableItems.Add(ls.StaticIndex);
...
#if DRAKKAR
	#region SHARED LISTS
	public sealed class SharedSimpleStaticList<T> : SimpleStaticList<T> where T : unmanaged
	{
		public NativeArray<T> Native;
		private SharedArray<T> Shared;
		public SharedSimpleStaticList(int max) : base(max)
		{
			Shared=new(max);
			Array=Shared;
			Native=Shared;
		...
		public NativeSlice<T> Slice => Native.Slice<T>(0,nextPosition);
	}

	public sealed class SharedStaticList<T,R> : StaticList<T> where T : unmanaged,IStaticLister where R : unmanaged
	{
		public NativeArray<R> Native;
		private SharedArray<T,R> Shared;
		public SharedStaticList(int max) : base(max)
		{
			Shared=new(max);
			Array=Shared;
			Native=Shared;
		...
		public NativeSlice<R> Slice => Native.Slice<R>(0,nextPosition);
	}

	public sealed class SharedSimpleStaticList<T,R> : SimpleStaticList<T> where T: unmanaged where R: unmanaged
	{
		public NativeArray<R> Native;
